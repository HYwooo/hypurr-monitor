"""Train LSTM+CatBoost: Train 2024-09~2025-08, Valid 2025-09, Test 2025-10, threshold=0.12"""
...
def compute_metrics(y_true, y_pred, y_proba):
    ...
    geo = np.sqrt(long_p * short_p)
    recall_avg = (long_r + short_r) / 2
    m["tcs"] = geo + 0.2 * recall_avg - 2.0 * (m["P_aS_pL"] + m["P_aL_pS"]) - 0.3 * (m["P_aL_pN"] + m["P_aS_pN"])
...
def train_and_eval(train_months, valid_month, test_month, fold_idx):
    print(
        f"\n{'=' * 60}\nFold {fold_idx}: Train {train_months[0]}-{train_months[-1]}, Valid {valid_month}, Test {test_month}\n{'=' * 60}"
    )
    ...
    for phase, idx in [("Valid", valid_idx), ("Test", test_idx)]:
        ...
        print(
            f"  {phase}: Macro F1={m['macro_f1']:.4f}, TCS={m['tcs']:.4f}, Neutral%={pd_dist.get(1, 0) / len(ypr) * 100:.1f}%"
        )
    return results
...
print(f"Train: {train_months[0]} to {train_months[-1]} ({len(train_months)} months), Valid: 202509, Test: 202510")

all_results = train_and_eval(train_months, 202509, 202510, 0)
...
| 指标 | 值 |
|------|-----|
| **Macro F1** | {v["macro_f1"]:.4f} |
| **TCS (New)** | {v["tcs"]:.4f} |
| **Neutral Ratio** | {v["pred_distribution"]["NEUTRAL"] / v["test_size"] * 100:.1f}% |
| **Long Recall** | {v["long_recall"]:.4f} |
| **Short Recall** | {v["short_recall"]:.4f} |
| **Long Precision** | {v["long_precision"]:.4f} |
| **Short Precision** | {v["short_precision"]:.4f} |
...
| **TCS (New)** | {r["tcs"]:.4f} |
| **Neutral Ratio** | {r["pred_distribution"]["NEUTRAL"] / r["test_size"] * 100:.1f}% |
| **Long Recall** | {r["long_recall"]:.4f} |
| **Short Recall** | {r["short_recall"]:.4f} |
| **Long Precision** | {r["long_precision"]:.4f} |
| **Short Precision** | {r["short_precision"]:.4f} |
...
|------------|-------------|----------------|--------------|
| Long | {r["P_aL_pL"] * 100:.1f}% | {r["P_aN_pL"] * 100:.1f}% | {r["P_aS_pL"] * 100:.1f}% |
| Neutral | {r["P_aL_pN"] * 100:.1f}% | {r["P_aN_pN"] * 100:.1f}% | {r["P_aS_pN"] * 100:.1f}% |
| Short | {r["P_aL_pS"] * 100:.1f}% | {r["P_aN_pS"] * 100:.1f}% | {r["P_aS_pS"] * 100:.1f}% |

**P(predicted|actual)**
| Actual | Pred Long | Pred Neutral | Pred Short |
|--------|-----------|--------------|------------|
| Long | {r["P_pL_aL"] * 100:.1f}% | {r["P_pN_aL"] * 100:.1f}% | {r["P_pS_aL"] * 100:.1f}% |
| Neutral | {r["P_pL_aN"] * 100:.1f}% | {r["P_pN_aN"] * 100:.1f}% | {r["P_pS_aN"] * 100:.1f}% |
| Short | {r["P_pL_aS"] * 100:.1f}% | {r["P_pN_aS"] * 100:.1f}% | {r["P_pS_aS"] * 100:.1f}% |
...
```
         Predicted
         DOWN    NEUTRAL  UP
Actual DOWN   {r["confusion_matrix"][0][0]}      {r["confusion_matrix"][0][1]}       {r["confusion_matrix"][0][2]}
       NEUTRAL {r["confusion_matrix"][1][0]}      {r["confusion_matrix"][1][1]}      {r["confusion_matrix"][1][2]}
       UP     {r["confusion_matrix"][2][0]}       {r["confusion_matrix"][2][1]}       {r["confusion_matrix"][2][2]}
```
